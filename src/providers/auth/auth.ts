...
export class AuthProvider {
  ...
  checkLogin() {
    this.storage.get(this.jwtTokenName).then(jwt => {
      if (jwt && !this.jwtHelper.isTokenExpired(jwt)) {
        this.httpClient.get(`${SERVER_URL}/auth/authenticate`)
  ...
  login(values: any): Observable<any> {
    return this.httpClient.post(`${SERVER_URL}/auth/login`, values, {responseType: 'json'})
  ...
  signup(values: any): Observable<any> {
    return this.httpClient.post(`${SERVER_URL}/user`, values, {responseType: 'json'})
      .pipe(tap(jwt => {
        if (jwt !== 'EXISTS') {
